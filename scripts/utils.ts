...
export const packages = fs
  .readdirSync(
    new URL('../packages', import.meta.url).toString().replace(/file:\/\//, ''),
  )
  .filter((name) => !name.startsWith('.'))

export const templates = ['packages/create-valaxy/template']
...
export function updateVersion(pkgPath: string, version: string): void {
  const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8'))
  consola.info(
    `${chalk.cyan(pkg.name)} ${chalk.gray(`v${pkg.version}`)} -> ${chalk.yellow(
      `v${version}`,
    )}`,
  )
  pkg.version = version
...
export function getVersionChoices(currentVersion: string) {
  ...
  const inc: (i: ReleaseType) => string = (i) => semver.inc(currentVersion, i)!
